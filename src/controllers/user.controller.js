...
const generateAccessAndRefreshTokens  = async(userId)=>{
   ...
   return {accessToken,refreshToken}
        ...
        throw new ApiError(500,"SOMETHING WROGN WHILE GENRATING REFRESH ND ACCES TOEKN")
    }
}
...
const loginUser = asyncHandler(async(req,res)=>{
    ...
    if (!username || email) {
        throw new ApiError(404,"username or pass is req")
    }
...
const isPasswordValid = await user.isPasswordCorrect(password)
 if (!isPasswordValid) {
        throw new ApiError(404,"passord incorrect")
    }
...
const changeCurrentPassword = asyncHandler(async(req,res)=>{
    ...
    if ((!newPassword===confPassword)) {
        throw new ApiError(400,"passwurd dont matches")
    }
    const user = await User.findById(req.user?._id)
    const isPasswordCorrect = await user.isPasswordCorrect(oldPassword)
    if (!isPasswordCorrect) {
        throw new ApiError(400,"Invalid old passs")
    }
    user.password = newPassword
    await user.save({validateBeforeSave:false})
    return res
    .status(200).json(new ApiResponse(200,{},"Password changed succccesssfullly"))
})
...
const getUserChannelProfile = asyncHandler(async(req,res)=>{
    ...
    const channel = await User.aggregate([
        ...
        },
        {
            ...
            as: "subscribedTo"//things we have subscribedd
          }  
        },
        {
            $addFields: {
    ...
    return res.status(200).json(200,channel)
    // console.log(channel)
})
